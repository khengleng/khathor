...
from twisted.internet.interfaces import IProtocol, IPushProducer, IReactorCore
...
class NodeBlockSync(SyncManager):
    ...
    def __init__(self, protocol: 'HathorProtocol', sync_checkpoints: SyncCheckpoint, reactor: Clock = None) -> None:
        """
    ...
    def get_status(self, debug: bool = False) -> Dict[str, Any]:
        ...
        if debug:
            debug_data: Dict[str, Any] = {
                'height_best_block': self.tx_storage.get_height_best_block(),
                'len_needed_txs': len(self.tx_storage._needed_txs_index),
                'len_dir_deps': len(self.tx_storage._dir_dep_index),
                'len_rev_deps': len(self.tx_storage._rev_dep_index),
            }
            if self.tx_storage.has_needed_tx():
                debug_data['needed_tx'] = self.tx_storage.get_next_needed_tx().hex()
            res['_debug'] = debug_data
        return res
    ...
    def _run_sync(self) -> Generator[Any, Any, None]:
        ...
        elif self.tx_storage.has_needed_tx():
    ...
    def run_sync_transactions(self) -> None:
        ...
        start_hash = self.tx_storage.get_next_needed_tx()
    ...
    def handle_get_tips(self, payload: str) -> None:
        ...
        for txid in self.tx_storage.get_mempool_tips_index():
            self.send_tips(txid)
    ...
    def find_best_common_block(self, peer_best_height: int, peer_best_block: bytes) -> Generator[Any, Any, None]:
        ...
        if peer_best_height <= my_best_height:
            my_block = self.tx_storage.get_from_block_height_index(peer_best_height)
    ...
    def handle_get_peer_block_hashes(self, payload: str) -> None:
        ...
        data = []
        for h in heights:
            block = self.tx_storage.get_from_block_height_index(h)
            if block is None:
                break
